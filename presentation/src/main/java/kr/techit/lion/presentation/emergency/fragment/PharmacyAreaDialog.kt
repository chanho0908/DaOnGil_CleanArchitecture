...
import kr.techit.lion.presentation.databinding.DialogEmergencyAreaBinding
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        with(binding) {
            ...
            emergencyAreaSelected.setOnItemClickListener { adapterView, _, position, _ ->
                ...
                @ArrayRes val detailAreasResourceId = resources.getIdentifier(
                    "area_details_$areaNameInEnglish", "array", context?.packageName
                )

                val detailAreas = resources.getStringArray(detailAreasResourceId)

                val areaDetailAdapter = ArrayAdapter(
                    requireContext(), android.R.layout.simple_dropdown_item_1line, detailAreas
                )
